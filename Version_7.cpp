...
#define TURNANGLE 10
#define ANGLE_OF_SENSORS 10
#define BRUSHSIZE 1 //(n+2) x (n+2) Feld gemalt
...
int main()
{
    ...
    for(int i=0; i<NUMBER_OF_ANTS; i++)
    {
        sensor_left[i].setRadius(7.f);
        sensor_left[i].setFillColor(sf::Color::Red);
        sensor_right[i].setRadius(7.f);
        sensor_right[i].setFillColor(sf::Color::Red);
        sensor_straight[i].setRadius(7.f);
        sensor_straight[i].setFillColor(sf::Color::Red);
    }
