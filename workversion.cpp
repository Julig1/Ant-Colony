...
/*
    ...
    - ants zu random location teleportieren
    - ants in die mitte teleportieren
    - Hindernisse programmieren
    - Screenshot funktion einbauen
...
#define MAX_X (500 + FOOD_SCANNER_FIELD)
#define MAX_Y (500 + FOOD_SCANNER_FIELD)
...
#define STEPS_TO_FIND_FOOD 1000 //after x steps return 
#define PHEROMONE_DECAY_TIMER 2
#define MARKER_THRESHHOLD 50 //when a pixel is brigher than this number in both marker marker colors ants will change direction towards it
...
int main()
{
    ...
    float marker_dir[NUMBER_OF_ANTS];
    float pheromone_intensity[NUMBER_OF_ANTS];
    float timer_food_finder[NUMBER_OF_ANTS];
    ...
    for(int i=0; i<NUMBER_OF_ANTS; i++)
    {
        ...
        timer_food_finder[i] = STEPS_TO_FIND_FOOD;
    }
    for(int i=0; i<NUMBER_OF_ANTS; i++)
    {
    ...
    // draw everything here...    
        ...
        for(int i=0; i<NUMBER_OF_ANTS; i++)
        {
            ...
            if (timer_food_finder[i] >= 0)
            {
                timer_food_finder[i] = timer_food_finder[i] - 1; //can be float
            }
            //position of ant i
            ...
            if (status_ant[i] == SEEKING_FOOD)
            {
                ...
                float minimum_food = MAX_X;
                float minimum_marker = MAX_X;
                float dimmest_pixel = 2*MARKER_THRESHHOLD;
                previous_dir[i] = dir[i];
                executed_finding_food = FALSE;
                for (int j=-FOOD_SCANNER_FIELD; j<(FOOD_SCANNER_FIELD+1); j++)
                {
                    for(int k=-FOOD_SCANNER_FIELD; k<(FOOD_SCANNER_FIELD+1); k++)
                    {
                        ...
                        int FM = food_scanner_field_color.TOFOOD_MARKER_COLOR;
                        int HM = food_scanner_field_color.TOHOME_MARKER_COLOR; 
                        float distance = sqrt(k*k+j*j);
                        if(F > 0)
                        {
                            if(distance < minimum_food)
                            {
                                minimum_food = distance;
                                if(k<0||k>0)
                                {
                                    float h = (float)j;
                                    float j_divide_k = h/k;
                                    //cout << minimum_food << ", new_dir="<< round(atan(j_divide_k)/RAD_PER_DEG) << ", j/k=" << j_divide_k << ", k=" << k << ", j=" << j << ", x=" << x << ", y=" << y << endl;
                        ...
                        /*if(FM > MARKER_THRESHHOLD && HM > MARKER_THRESHHOLD)
                        {
                            float merged_color = FM + HM;
                            if(merged_color > dimmest_pixel)
                            {
                                dimmest_pixel = merged_color;
                                if(k<0||k>0)
                                {
                                    float h = (float)j;
                                    float j_divide_k = h/k;
                                    marker_dir[i] = atan(j_divide_k)/RAD_PER_DEG;
                                    if(j<TRAILSIZE && k<TRAILSIZE)
                                    {
                                        marker_dir[i]= marker_dir[i] + 180;
                                    }
                                    else if(j>TRAILSIZE && k>TRAILSIZE)
                                    {
                                        marker_dir[i]= marker_dir[i];
                                    }
                                    else if(j<TRAILSIZE && k>TRAILSIZE)
                                    {
                                        marker_dir[i]= marker_dir[i];
                ...
                /*if(executed_finding_food == FALSE)
                {
                    float A = (dir[i]-10);
                    float B = (dir[i] + 10);
                    if(A < marker_dir[i] > B)
            ...
            else if (status_ant[i] == GOING_HOME)
            {
                ...
                if(timer_food_finder[i] == 0)
                {
                    //marker_dir[i] = atan(j_divide_k)/RAD_PER_DEG;//dir[i] zeigt zu home         
                }
        ...
        string(text_display) = "Essen in Colonie: " + to_string(food_in_colony) + "    Anzahl der Ameisen: " + to_string(NUMBER_OF_ANTS);
        sf::Text food_text(text_display, Arial);
        food_text.setCharacterSize(20);
        food_text.setStyle(sf::Text::Bold);
        food_text.setFillColor(sf::Color::White);
        food_text.setPosition(FOOD_SCANNER_FIELD, FOOD_SCANNER_FIELD - 30);
